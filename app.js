...
let cardImagePaths = [...CARD_IMAGE_PATHS];
...
function createDeck() {
  return shuffleArray(
    cardImagePaths.flatMap((image, index) => {
...
function getFallbackCardImagePath(image) {
  if (image.startsWith("/Deck/")) {
    return image.replace("/Deck/", "/");
  }
  return image;
}
...
async function init() {
  cardImagePaths = await loadCardImagePaths();
...
async function loadCardImagePaths() {
  try {
    ...
    console.log("[deck:manifest:loaded]", {
      ...
      sample: payload.cards.slice(0, 5),
    });
    return payload.cards.map((image) => toPublicAssetPath(image));
  } catch (error) {
    console.warn("[deck:manifest:fallback]", error);
    return CARD_IMAGE_PATHS.map((image) => toPublicAssetPath(image));
  }
}
